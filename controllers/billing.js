...
exports.createBill = async (req, res) => {
  try {
    ...
    const { patient_id_ref, ipd_admission_id_ref, visit_id_ref, items, generated_by_user_id } = req.body;

if (!patient_id_ref || !items || !generated_by_user_id) {
  return res.status(400).json({ message: 'Required fields missing.' });
}

// ✅ At least one must exist
if (!ipd_admission_id_ref && !visit_id_ref) {
  return res.status(400).json({ message: 'Either IPD or OPD (visit) required.' });
}
   ...
   else if (item_type === 'OPDConsultation') {
  const opd = await OPDConsultation.findById(item_source_id);

  if (!opd) return res.status(404).json({ message: 'OPD not found.' });

  unit_price = 300; // ya dynamic consultation fee
  desc = `OPD Consultation`;
}
   
   
   
   
        else if (item_type === 'Sonography') {
  const scan = await Sonography.findById(item_source_id).populate('patientId');

  if (!scan) {
    return res.status(404).json({ message: 'Sonography not found.' });
  }

  if (scan.isBilled) {
    return res.status(400).json({ message: 'This scan already billed.' });
  }

  unit_price = scan.cost || 0;
  desc = `Sonography - ${scan.scanType}`;

  // mark later
  billedProcedureIds.push(scan._id);
}
    ...
    const bill = new Bill({
      ...
      ipd_admission_id_ref: ipd_admission_id_ref || null,
      visit_id_ref: visit_id_ref || null,
